...
from .upscaling import (
    dispatch as dispatch_upscaling,
    prepare as prepare_upscaling,
    unload as unload_upscaling,
)
from .ocr import dispatch as dispatch_ocr, prepare as prepare_ocr, unload as unload_ocr
from .textline_merge import dispatch as dispatch_textline_merge
from .mask_refinement import dispatch as dispatch_mask_refinement
from .inpainting import (
    dispatch as dispatch_inpainting,
    prepare as prepare_inpainting,
    unload as unload_inpainting,
)
...
class MangaTranslator:
    ...
    async def _batch_translate(self, config: Config, ctx: Context) -> Context:
        ...
        for img in images:
            ...
            for textline in local_ctx.textlines:
        ...
        for local_ctx in contexts:
            count = len(local_ctx.text_regions) if local_ctx.text_regions else 0
    ...
    async def translate(self, image: Image.Image, config: Config, zip_file: bytes = None) -> Context:
        ...
        """
        if zip_file:
            return await self.batch_translate(zip_file, config)
    ...
    async def _run_colorizer(self, config: Config, ctx: Context):
        ...
        self._model_usage_timestamps[("colorizer", config.colorizer.colorizer)] = (
            current_time
        )
    ...
    async def _run_upscaling(self, config: Config, ctx: Context):
        current_time = time.time()
        self._model_usage_timestamps[("upscaling", config.upscale.upscaler)] = (
            current_time
        )
        return (
            await dispatch_upscaling(
    ...
    async def _run_detection(self, config: Config, ctx: Context):
        current_time = time.time()
        self._model_usage_timestamps[("detection", config.detector.detector)] = (
            current_time
        )
        ...
        for textline in textlines:
            if textline.text.strip():
                if config.render.font_color_fg:
                    textline.fg_r, textline.fg_g, textline.fg_b = (
                        config.render.font_color_fg
                    )
                if config.render.font_color_bg:
                    textline.bg_r, textline.bg_g, textline.bg_b = (
                        config.render.font_color_bg
                    )
    ...
    async def _run_textline_merge(self, config: Config, ctx: Context):
        ...
        for region in text_regions:
            ...
            removed_start_chars = original_text[
                : len(original_text) - len(stripped_text)
            ]
            ...
            if (
                ...
                or (
                    not config.translator.no_text_lang_skip
                    and langcodes.tag_distance(
                        region.source_lang, config.translator.target_lang
                    )
                    == 0
                )
            ):
    ...
    async def _run_text_translation(self, config: Config, ctx: Context, reset_samples: bool = False):
        ...
        if self.load_text:
            ...
            with open(
                self._result_path(f"{input_filename}_translations.txt"), "r"
            ) as f:
            ...
            if self.save_text:
                input_filename = os.path.splitext(
                    os.path.basename(self.input_files[0])
                )[0]
                with open(
                    self._result_path(f"{input_filename}_translations.txt"), "w"
                ) as f:
        ...
        if config.translator.target_lang in special_langs:
            ...
            same_target_regions = (
                []
            )  # Target language regions with identical translation
            diff_target_regions = (
                []
            )  # Target language regions with different translation
            same_non_target_regions = (
                []
            )  # Non-target language regions with identical translation
            diff_non_target_regions = (
                []
            )  # Non-target language regions with different translation
            ...
            for region in ctx.text_regions:
                ...
                if config.translator.target_lang in ["CHS", "CHT"]:  # Chinese
                    ...
                    has_target_lang_in_translation = bool(
                        re.search("[\u4e00-\u9fff]", region.translation)
                    )
                elif config.translator.target_lang == "JPN":  # Japanese
                    has_target_lang = bool(
                        re.search(
                            "[\u3040-\u309f\u30a0-\u30ff\u4e00-\u9fff]", region.text
                        )
                    )
                    ...
                    has_target_lang_in_translation = bool(
                        re.search("[A-Za-z]", region.translation)
                    )
                ...
                if config.filter_text and re.search(
                    config.re_filter_text, region.translation
                ):
                    logger.info(f"Filtered out: {region.translation}")
            ...
            for region in diff_non_target_regions:
                if region in has_target_lang_in_translation_regions:
                    ...
                    logger.info(f"Filtered out: {region.translation}")
                    logger.info(
                        "Reason: Translation does not contain target language characters"
                    )
            ...
            if not (diff_target_regions or diff_non_target_regions):
                for region in same_target_regions:
                    logger.info(f"Filtered out: {region.translation}")
                    logger.info(
                        "Reason: Translation identical to original -the whole page-"
                    )
            ...
            for region in ctx.text_regions:
                ...
                if not config.translator.translator == Translator.none:
                    if region.translation.isnumeric():
                    ...
                    elif config.filter_text and re.search(
                        config.re_filter_text, region.translation
                    ):
                        should_filter = True
                        ...
                        text_equal = (
                            region.text.lower().strip()
                            == region.translation.lower().strip()
                        )
                        if text_equal:
                            should_filter = True
    ...
    async def _run_inpainting(self, config: Config, ctx: Context):
        current_time = time.time()
        self._model_usage_timestamps[("inpainting", config.inpainter.inpainter)] = (
            current_time
        )
    ...
    async def _run_text_rendering(self, config: Config, ctx: Context):
        current_time = time.time()
        self._model_usage_timestamps[("rendering", config.render.renderer)] = (
            current_time
        )
        ...
        # manga2eng currently only supports horizontal left to right rendering
            ...
            and ctx.text_regions
            and LANGUAGE_ORIENTATION_PRESETS.get(ctx.text_regions[0].target_lang) == "h"
        ):
