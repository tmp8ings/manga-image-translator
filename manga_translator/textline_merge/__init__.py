...
def split_text_region(
    ...
    if len(connected_region_indices) == 1:
        logger.debug(f"split_text_region: Only one index ({connected_region_indices[0]}). Returning merged region.")
    ...
    if len(connected_region_indices) == 2:
        ...
        if distance < (1 + gamma) * fs and angle_diff < 0.2 * np.pi:
            logger.debug(f"split_text_region: Merging two bboxes as distance {distance:.2f} < {(1+gamma)*fs:.2f} and angle difference {angle_diff:.2f} < {0.2*np.pi:.2f}.")
            return [set(connected_region_indices)]
        else:
            logger.debug(f"split_text_region: Not merging two bboxes as distance {distance:.2f} and angle difference {angle_diff:.2f} exceed thresholds. Splitting them.")
    ...
    if (distances_sorted[0] <= distances_mean + distances_std * sigma \
        ...
        logger.debug(f"split_text_region: Merging connected region indices as top edge weight {distances_sorted[0]:.2f} meets thresholds (mean {distances_mean:.2f}, std {distances_std:.2f}, fontsize {fontsize:.2f}).")
        return [set(connected_region_indices)]
    else:
        logger.debug(f"split_text_region: Splitting text region as top edge weight {distances_sorted[0]:.2f} exceeds thresholds (mean {distances_mean:.2f}, std {distances_std:.2f}, std_threshold {std_threshold:.2f}, max_poly_distance {max_poly_distance:.2f}, max_centroid_alignment {max_centroid_alignment:.2f}).")
        G = nx.Graph()
        for idx in connected_region_indices:
            G.add_node(idx)
