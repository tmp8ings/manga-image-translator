import os
from typing import Any, Dict, List, Callable, Protocol, Tuple, Optional
import numpy as np
import cv2
import functools
...
class Context(dict, ContextType):
...
def download_url_with_progressbar(url: str, path: str):
    ...
    if downloaded_size and r.headers.get("Accept-Ranges") != "bytes":
        print(
            "Error: Webserver does not support partial downloads. Restarting from the beginning."
        )
...
def resize_keep_aspect(img, size):
    ...
    return cv2.resize(
        img, (new_width, new_height), interpolation=cv2.INTER_LINEAR_EXACT
    )
...
def quadrilateral_can_merge_region(
    ...
    # logger.debug(
    #     f"quadrilateral_can_merge_region({a_text}, {b_text}): Return False - default case"
    # )
    return False
...
def get_color_name(rgb: List[int]) -> str:
    try:
        ...
        url = (
            f"https://www.thecolorapi.com/id?format=json&rgb={rgb[0]},{rgb[1]},{rgb[2]}"
        )
...
def det_rearrange_forward(
    ...
    if verbose:
        print(
            f"Input image will be rearranged to square batches before fed into network.\
...
def main():
    ...
    offset = 0
    s2 = [
        Point(1 + offset, 1 + offset),
        Point(1 + offset, 3 + offset),
        Point(3 + offset, 3 + offset + 1.5),
        Point(3 + offset + 1.5, 3 + offset),
        Point(3 + offset, 1 + offset),
    ]
